...
export class BookingController {
  ...
  async createBooking(
    req: Request<{}, {}, BookingSaveDTO>,
    res: Response<BookingResultDTO | ErrorResponse>
  ) {
    try {
  ...
  async previewBooking(
    ...
    try {
      const { guestSeq, start, end, rooms } = req.body;

      const booking = await this.bookingService.previewBooking(
        guestSeq,
        new Date(start),
        new Date(end),
        rooms
